"""
Title: kmnist_LeNet.py
Description: The network for Japanese MNIST, suitable for Deep SVDD or Deep SAD.
...
# 2. Decoder
# #########################################################################
class KMNISTLeNetDecoder(BaseNet):
    def __init__(self, rep_dim=64):
        super().__init__()
...
# 3. Autoencoder
# #########################################################################
class KMNISTLeNetAutoencoder(BaseNet):
    def __init__(self, rep_dim=64):
        super().__init__()

        self.rep_dim = rep_dim
        self.encoder = KMNISTLeNet(rep_dim=rep_dim)
        self.decoder = KMNISTLeNetDecoder(rep_dim=rep_dim)
