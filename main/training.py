...
for label_eval in label_eval_list:
    ...
    if label_eval[0] in label_normal:
        ...
        # Get binary label for both normal and abnormal data in order to calculate roc_auc_score
        # The following roc auc calculation does not work for semi-supervised setting. Need fix.
        # labels_all = [0] * len(labels_) + [1] * len(labels)
        # scores_all = list(scores_[:])
        # scores_all.extend(scores)
        # roc_auc = roc_auc_score(labels_all, scores_all)
        # total_roc_auc.append(roc_auc)
        # msg = '[ROC AUC score for class {}] {}\n'.format(label_eval[0], roc_auc)
        # f.write(msg)
        # print(msg)
...
# Save averaged roc_auc
# mean_roc_auc = np.array(total_roc_auc).mean()
# std_roc_auc = np.array(total_roc_auc).std()
# f.write('\n[**ROC AUC Mean**] {}\n[**ROC AUC std**] {}\n\n'.format(mean_roc_auc, std_roc_auc))
# print('\n[**ROC AUC Mean**] {}\n[**ROC AUC std**] {}\n\n'.format(mean_roc_auc, std_roc_auc))
