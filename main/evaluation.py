...
parser.add_argument('-le', '--loader_eval_name', type=str, default='fmnist_eval',
                    help='fmnist_eval, cifar10_eval')
parser.add_argument('-rt', '--root', type=str, default='/net/leksai/data/FashionMNIST',
                    help='[Choice]: /net/leksai/data/FashionMNIST, /net/leksai/data/CIFAR10')
...
if optimizer_ in ['one_class', 'one_class_unsupervised']:
    ...
    out_path = '/net/leksai/nips/model/one_class/{}/{}'.format(loader_name, folder_name)
    ...
    out_path = '/net/leksai/nips/model/rec/{}/{}'.format(loader_name, folder_name)
    final_path = '{}/net_{}_eta_{}_epochs_{}_batch_{}'.format(out_path, net_name, eta_str,
                                                              n_epochs, batch_size)
...
# Define the path for recall results
# We assume that you will have only one label_normal
# And we assume there in only 10 classes in total
int_label_normal = label_normal[0]
index = list(range(10))
index.pop(int_label_normal)
if loader_name[0] == loader_eval_name[0]:
    recall_path = '/net/leksai/nips/result/{}'.format(loader_name)
else:
    recall_path = '/net/leksai/nips/result/{}_{}'.format(loader_name, loader_eval_name)
if not os.path.exists(recall_path): os.makedirs(recall_path)
recall_df_90_path = Path(recall_path) / 'recall_df_{}_90.pkl'.format(int_label_normal)
recall_df_95_path = Path(recall_path) / 'recall_df_{}_95.pkl'.format(int_label_normal)
recall_df_99_path = Path(recall_path) / 'recall_df_{}_99.pkl'.format(int_label_normal)
default_recall_df = pd.DataFrame(np.zeros((9, 1)),
                                 index=index,
                                 columns=pd.MultiIndex.from_product([['placeholder'],
                                                                     ['placeholder']]))

if not recall_df_90_path.is_file():
    print ('Creating a new path for recall dataframe 90!')
    recall_df_90 = default_recall_df.copy(deep=True); recall_df_90.to_pickle(recall_df_90_path)
if not recall_df_95_path.is_file():
    print ('Creating a new path for recall dataframe 95!')
    recall_df_95 = default_recall_df.copy(deep=True); recall_df_95.to_pickle(recall_df_95_path)
if not recall_df_99_path.is_file():
    print ('Creating a new path for recall dataframe 99!')
    recall_df_99 = default_recall_df.copy(deep=True); recall_df_99.to_pickle(recall_df_99_path)

recall_df_90 = pd.read_pickle(recall_df_90_path)
recall_df_95 = pd.read_pickle(recall_df_95_path)
recall_df_99 = pd.read_pickle(recall_df_99_path)
...
for label_eval in label_eval_list:
    ...
    #     test_eval = True
    # De-comment the above two lines and comment out this line if want all set.
    test_eval = True

    f.write('============================================================\n')
    f.write('Results for Class {}:\n'.format(label_eval))
    f.write('Dataframes has been pickled to {}!'.format(recall_df_90_path))
    ...
    print('I am loading model from {}, and evaluating data from {}.'.format(model_path, root))
    f.write('I am loading model from {}, and evaluating data from {}.'.format(model_path, root))
...
# Establish the index for the recall dataframe
if optimizer_ == 'rec_unsupervised': recall_ind = ('Reconstruction Model', 'A/N = 0')
if optimizer_ == 'one_class_unsupervised': recall_ind = ('One Class Model', 'A/N = 0')
if optimizer_ == 'rec': recall_ind = ('Reconstruction Model', 'A/N = {}, Abnormal: {}'.format(ratio_abnormal, label_abnormal[0]))
if optimizer_ == 'one_class': recall_ind = ('One Class Model', 'A/N = {}, Abnormal: {}'.format(ratio_abnormal, label_abnormal[0]))

# Save averaged results when FP = 0.10
recall_df_90.loc[:, recall_ind] = total_recall_90
...
recall_df_95.loc[:, recall_ind] = total_recall_95
...
recall_df_99.loc[:, recall_ind] = total_recall_99
...
try:
    recall_df_90.drop(('placeholder', 'placeholder'), axis=1, inplace=True)
    recall_df_95.drop(('placeholder', 'placeholder'), axis=1, inplace=True)
    recall_df_99.drop(('placeholder', 'placeholder'), axis=1, inplace=True)
except: print('Placeholder has been deleted!')

recall_df_90.to_pickle(recall_df_90_path)
recall_df_95.to_pickle(recall_df_95_path)
recall_df_99.to_pickle(recall_df_99_path)
